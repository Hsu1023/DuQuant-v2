...
def smooth_and_quant_inplace(model, args):
    ...
    #     #   We need original weights to calculate errors for GPTQ
        for _, module in model.named_modules():
            if isinstance(module, QuantLinear):
                # module.weight = module.weight_quantizer(module.weight, return_no_rotation=False)
                module.use_temporary_parameter = False

        
@torch.no_grad()
def quant_inplace(model, args):
    if not args.gptq:
        for name, module in model.named_modules():
            if isinstance(module, QuantLinear):
                module.weight = module.weight_quantizer(module.weight, return_no_quant=False, return_no_rotation=True)
