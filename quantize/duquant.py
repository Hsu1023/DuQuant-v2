...
def get_named_linears(module):
    return {
        name: m
        for name, m in module.named_modules() if isinstance(m, QuantLinear)
    }
...
def duquant(
    ...
    if "llama" in args.net.lower() or "vicuna" in args.net.lower(
    ) or "mistral" in args.net.lower() or "qwen" in args.net.lower():
    ...
    if args.resume:
        duquant_parameters = torch.load(
            os.path.join(args.resume, f"duquant_parameters.pth"))
    else:
        duquant_parameters = {}
    ...
    for i in range(len(layers)):
        for name in ['q', 'k', 'v', 'gate', 'up', 'down', 'o']:
            exec(
                f"args.{name}_weight_quant_params = copy.copy(args.weight_quant_params)"
            )
            exec(
                f"args.{name}_act_quant_params = copy.copy(args.act_quant_params)"
            )
        ...
        if "llama" in args.net.lower() or "vicuna" in args.net.lower():
            qlayer = DecoderLayer(lm.model.config, layer, args, i)
        else:
            qlayer = DecoderLayer(lm.model.config, layer, args, i)
                ...
                for name, module in qlayer.named_modules():
                    if isinstance(module, QuantLinear):
                        for key in pairs.keys():
                            if key in name:
                                act = act_scales[
                                    f"{layer_name_prefix}.{i}.{name}"].to(
                                        device=dev,
                                        dtype=dtype).clamp(min=CLIPMIN)
                                weight = module.weight.abs().max(
                                    dim=0)[0].clamp(min=CLIPMIN)
                                scale = (act.pow(args.alpha) / weight.to(
                                    act.device).pow(1 - args.alpha)).clamp(
                                        min=CLIPMIN)
        ...
        if args.smooth_epochs > 0:
            ...
            optimizer = torch.optim.AdamW([
                {
                    "params": smooth_parameters(qlayer, use_shift),
                    "lr": args.let_lr
                },
            ],
            ...
            for epochs in range(args.smooth_epochs):
                ...
                for j in range(args.nsamples // args.batch_size):
                    ...
                    norm = loss_scaler(
                        loss, optimizer,
                        parameters=smooth_parameters(qlayer)).cpu()
        ...
        if args.let:
            ...
            if duquant_parameters.get(i):
                ...
                qlayer.register_parameter(
                    "qkt_post_scale",
                    torch.nn.Parameter(
                        torch.ones(layer.self_attn.q_proj.out_features,
                                   device=dev,
                                   dtype=dtype)))
                for name, module in qlayer.named_modules():
                    if isinstance(module, QuantLinear):
                        for key in pairs.keys():
                            if key in name:
                                act = module.act_quantizer.recorded_x_max.clamp(
                                    min=CLIPMIN)
                                weight = module.weight_quantizer.recorded_x_max.clamp(
                                    min=CLIPMIN)
                                scale = (act.pow(args.let_alpha) / weight.to(
                                    act.device).pow(1 - args.let_alpha)).clamp(
                                        min=0.8)
                                if key not in ['down_proj']:
                                    qlayer.register_parameter(
                                        f"{pairs[key]}_post_scale",
                                        torch.nn.Parameter(
                                            scale, requires_grad=False))
                                else:
                                    qlayer.register_parameter(
        ...
        if args.epochs > 0:
            ...
            optimizer = torch.optim.AdamW([
                {
                    "params": let_parameters(qlayer, use_shift),
                    "lr": args.let_lr
                },
                {
                    "params": lwc_parameters(qlayer),
                    "lr": args.lwc_lr
                },
            ],
                                          weight_decay=args.wd)
            ...
            for epochs in range(args.epochs):
                ...
                original_parameters = [
                    param.clone() for param in get_post_parameters(qlayer)
                ]
                loss_list = []
                norm_list = []
                ...
                for j in range(args.nsamples // args.batch_size):
                    ...
                    norm = loss_scaler(
                        loss,
                        optimizer,
                        parameters=get_post_parameters(qlayer)).cpu()
        ...
        if args.epochs > 0:
            ...
            if args.save_dir:
                torch.save(
                    duquant_parameters,
                    os.path.join(args.save_dir, f"duquant_parameters.pth"))
        else:
            register_scales_and_zeros(qlayer)
            layers[i] = qlayer.to("cpu")
            duquant_parameters[i] = duquant_state_dict(qlayer)
