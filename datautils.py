...
def get_pile(nsamples, seed, seqlen, model):
    ...
    raise NotImplementedError(
        "The Pile dataset requires a local copy of val.jsonl.zst. "
        "Please download it and modify this function with the correct path."
    )
...
def get_wikitext2(nsamples, seed, seqlen, model):
    ...
    # trainenc = tokenizer("\n\n".join(traindata['text']), return_tensors='pt', truncation=True, max_length=131072)
    # testenc = tokenizer("\n\n".join(testdata['text']), return_tensors='pt', truncation=True, max_length=131072)
...
def get_c4_new(nsamples, seed, seqlen, model):
    ...
    for _ in range(nsamples):
        while True:
            i = random.randint(0, len(traindata) - 1)
            trainenc = tokenizer(traindata[i]["text"], return_tensors="pt")
            # trainenc = tokenizer(traindata[i]["text"], return_tensors="pt", truncation=True, max_length=131072)
            if trainenc.input_ids.shape[1] >= seqlen:
                break
        i = random.randint(0, trainenc.input_ids.shape[1] - seqlen - 1)
        j = i + seqlen
    ...
    # valenc = tokenizer(" ".join(valdata[:1100]["text"]), return_tensors="pt", truncation=True, max_length=131072)
