...
class QuantLlamaAttention(nn.Module):
    ...
    def get_sample_num(cls):
        cls.num_of_global_layer += 1
        cls.num_of_samples = (cls.num_of_global_layer - 1) // 28
        return cls.num_of_samples, cls.num_of_global_layer
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Compatible with both old and new transformers API
        import inspect
        _apply_rope_params = inspect.signature(apply_rotary_pos_emb).parameters
        if 'unsqueeze_dim' in _apply_rope_params and 'position_ids' not in _apply_rope_params:
        ...
        if attention_mask is not None:
            attention_mask = attention_mask[:, :, :, : key_states.shape[-2]]   # for Llama-3.2-3B
...
class QuantLlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, ori_layer, args, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = QuantLlamaAttention(org_module=ori_layer.self_attn,
                                             config=config,
                                             args=args,
                                             layer_idx=layer_idx)
        ...
        self.input_layernorm = DuLlamaRMSNorm(
            ori_layer.input_layernorm,
            eps=ori_layer.input_layernorm.variance_epsilon)
        self.post_attention_layernorm = DuLlamaRMSNorm(
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.mlp(
            hidden_states.to(self.mlp.up_proj.weight.device)).to(
                residual.device)
        hidden_states = residual + hidden_states
    ...
    def register_duquant_params(self):
        for name, module in self.named_modules():
            if isinstance(module, QuantLlamaMLP) or isinstance(
                    module, QuantLlamaAttention):
